				   ...
				   struct v4l2_subdev_format *fmt)
{
	...
	if (fmt->which == V4L2_SUBDEV_FORMAT_TRY) {
        //debug_printk("__veyecam2m_get_pad_format====V4L2_SUBDEV_FORMAT_TRY===");
                ...
                fmt->format = *v4l2_subdev_get_try_format(&veyecam2m->subdev, sd_state, fmt->pad);
			   ...
			   struct v4l2_subdev_format *fmt)
{
    ...
    VEYE_TRACE
	mutex_lock(&veyecam2m->mutex);
	ret = __veyecam2m_get_pad_format(veyecam2m, state, fmt);
	mutex_unlock(&veyecam2m->mutex);
	return ret;
}
...
static int veyecam2m_g_mbus_config(struct v4l2_subdev *sd,  unsigned int pad_id,
...
//		veyecam2m_lanes = veyecam2m->lane_data_num;
  ...
  //      V4L2_MBUS_CSI2_NONCONTINUOUS_CLOCK ;//discontinues mode
	...
	config->bus.mipi_csi2.num_data_lanes = veyecam2m->lane_data_num;
//        config->flags = val;
...
static const struct v4l2_subdev_pad_ops veyecam2m_pad_ops = {
	...
	.get_mbus_config = veyecam2m_g_mbus_config,
