...
static int gxcam_g_mbus_config(struct v4l2_subdev *sd, unsigned int pad_id,
				struct v4l2_mbus_config *config)
{
	//u32 val = 0;
    struct gxcam *gxcam = to_gxcam(sd);
	//val = 1 << (gxcam->lane_num - 1) |
	//      V4L2_MBUS_CSI2_CHANNEL_0 |
	//      V4L2_MBUS_CSI2_CONTINUOUS_CLOCK;
	config->type = V4L2_MBUS_CSI2_DPHY;
	config->bus.mipi_csi2.num_data_lanes = gxcam->lane_num;
...
static int gxcam_g_frame_interval(struct v4l2_subdev *sd,
				    struct v4l2_subdev_frame_interval *fi)
{
    ...
    fract_fps.denominator = gxcam->cur_fps*10000;
...
static int gxcam_s_frame_interval(struct v4l2_subdev *sd,
				    struct v4l2_subdev_frame_interval *fi)
{
	...
	mutex_lock(&gxcam->mutex);
    gxcam->cur_fps = fi->interval.denominator/fi->interval.numerator;
    gxcam_write(gxcam->client, Framerate_ex,gxcam->cur_fps*10000);
	mutex_unlock(&gxcam->mutex);
...
{
	//MEDIA_BUS_FMT_SENSOR_DATA not found on rk
								...
								struct v4l2_subdev_format *format)
{
    ...
    if (format->which == V4L2_SUBDEV_FORMAT_TRY) {
            ...
            format->format = *v4l2_subdev_get_try_format(&gxcam->sd, sd_state, format->pad);
        #else
				...
				struct v4l2_subdev_selection *sel)
{
	...
	case V4L2_SEL_TGT_CROP: {
			mutex_lock(&gxcam->mutex);
			sel->r = *__gxcam_get_pad_crop(sd, gxcam, cfg, sel->pad, sel->which);
			mutex_unlock(&gxcam->mutex);
    		break;
    	}
								...
								struct v4l2_subdev_format *format)
{
	...
	if (format->which == V4L2_SUBDEV_FORMAT_TRY) {
			framefmt = v4l2_subdev_get_try_format(&gxcam->sd, sd_state, format->pad);
...
static void gxcam_get_module_inf(struct gxcam *gxcam,
				   struct rkmodule_inf *inf)
{
    VEYE_TRACE
	memset(inf, 0, sizeof(*inf));
	strlcpy(inf->base.sensor, gxcam_NAME, sizeof(inf->base.sensor));
	strlcpy(inf->base.module, gxcam->module_name,
		sizeof(inf->base.module));
	strlcpy(inf->base.lens, gxcam->len_name, sizeof(inf->base.lens));
}

static int gxcam_get_channel_info(struct gxcam *gxcam, struct rkmodule_channel_info *ch_info)
{
        struct gxcam_format *current_format;
...
static long gxcam_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	struct gxcam *gxcam = to_gxcam(sd);
	long ret = 0;
	...
	case RKMODULE_GET_MODULE_INFO:
		gxcam_get_module_inf(gxcam, (struct rkmodule_inf *)arg);
		break;
    case RKMODULE_GET_CHANNEL_INFO:
		ch_info = (struct rkmodule_channel_info *)arg;
		ret = gxcam_get_channel_info(gxcam, ch_info);
		break;
...
#ifdef CONFIG_COMPAT
static long gxcam_compat_ioctl32(struct v4l2_subdev *sd,
				   unsigned int cmd, unsigned long arg)
{
    ...
    case RKMODULE_GET_CHANNEL_INFO:
		...
		ret = gxcam_ioctl(sd, cmd, ch_info);
		if (!ret) {
...
static int gxcam_power_on(struct device *dev)
{
    ...
    gpiod_set_value_cansleep(gxcam->pwdn_gpio, 1);
...
static int gxcam_power_off(struct device *dev)
{
	...
	gpiod_set_value_cansleep(gxcam->pwdn_gpio, 1);//still use 1
	gpiod_set_value_cansleep(gxcam->reset_gpio, 0);
...
static const struct v4l2_subdev_core_ops gxcam_core_ops = {
    ...
    .ioctl = gxcam_ioctl,
#ifdef CONFIG_COMPAT
    .compat_ioctl32 = gxcam_compat_ioctl32,
#endif
};

static const struct v4l2_subdev_video_ops gxcam_video_ops = {
	.s_stream = gxcam_set_stream,
    .g_frame_interval = gxcam_g_frame_interval,
    .s_frame_interval = gxcam_s_frame_interval, 
};
...
static const struct v4l2_subdev_pad_ops gxcam_pad_ops = {
	...
	.get_mbus_config = gxcam_g_mbus_config,
...
static void free_gpio(struct gxcam *gxcam)
{
	if (!IS_ERR(gxcam->pwdn_gpio))
		gpio_free(desc_to_gpio(gxcam->pwdn_gpio));
    if (!IS_ERR(gxcam->reset_gpio))
        gpio_free(desc_to_gpio(gxcam->reset_gpio));
    //    if (!IS_ERR(gxcam->mipi_pwr_gpio))
	//	gpio_free(desc_to_gpio(gxcam->mipi_pwr_gpio));
}
			...
			const struct i2c_device_id *id)
{
    ...
    gxcam->reset_gpio = devm_gpiod_get(dev, "reset", GPIOD_OUT_HIGH);
	if (IS_ERR(gxcam->reset_gpio)) {
		dev_info(dev, "Failed to get reset-gpios, maybe no use\n");
	}

	gxcam->pwdn_gpio = devm_gpiod_get(dev, "pwdn", GPIOD_OUT_HIGH);
	if (IS_ERR(gxcam->pwdn_gpio)) {
	...
	if (strcmp(gxcam->module_facing, "back") == 0)
		facing[0] = 'b';
	else
		facing[0] = 'f';

	snprintf(gxcam->sd.name, sizeof(gxcam->sd.name), "m%02d_%s_%s %s",
		 gxcam->module_index, facing,
		 gxcam_NAME, dev_name(gxcam->sd.dev));
	...
	ret = v4l2_async_register_subdev(&gxcam->sd);
	if (ret){
...
static const struct of_device_id gxcam_of_match[] = {
...
static struct i2c_driver veyegx_cam_i2c_driver = {
	.driver = {
		.name = "gxcam",
		.of_match_table	= of_match_ptr(gxcam_of_match),
	},
	.probe = gxcam_probe,
	.remove = gxcam_remove,
	.id_table = gxcam_match_id,
};

static int __init sensor_mod_init(void)
{
	return i2c_add_driver(&veyegx_cam_i2c_driver);
}

static void __exit sensor_mod_exit(void)
{
	i2c_del_driver(&veyegx_cam_i2c_driver);
}
